         ...
         $(document).ready(function(){
            ...
            $("#tabla1 tbody").on("click", '.mas', function () {
               ...
               if(evento_id.indexOf("fila") > -1){ //si el id contiene la palabra fila significa que se quiere programar un nuevo turno
                  ...
                  var cant_personal=0;//detecto cada vez que alguien selecciona un personal que brinda un servicio
                  ...
                  $("#modal_nuevo_evento").off("change", ".select_servicios").on("change", ".select_servicios", function(){
                     ...
                     if (fila.find('td:eq(0) select').val()==""){
                        ...
                        // Listo los pesonales por servicio
                        // function listarPersonal(datos_personal_eventos) {
                        ...
                        //       dataType: 'json',
                        //       async: false,
                        //       //la respuesta de este ajax se guarda en personal
                        //       success: function (personal) {
                        //          var input = '';
                        //          input += '<td>';
                        //          //la respuesta de ajax se guarda en datos_personal
                        //          datos_personal.personal.forEach(function (e) {
                        //            // var comprueba_personal = true;
                        //             cant_personal++;
                        //             input += "<td><input class='form-check-input' type='checkbox' value='" + e.usuario_id + "' name='" + e.nombre_animador + " " + e.apellido_animador + cant_personal + "'><label class='form-check-label' for='" + e.servicio_id + "personal" + cant_personal + "'>" + e.nombre_animador + "</label><br></td>";
                        //             // input +="<td><input type='time' class='form-control border-primary'  name='"+servicio_id+"hora_inicio' ></td>";
                        //             // input +="<td><input type='time' class='form-control border-primary'  name='"+servicio_id+"hora_inicio' ></td>";
                        ...
                        //             // if (comprueba_personal) {
                        //             // } else {
                        //             //    input += "<input class='form-check-input' type='checkbox' disabled value='" + e.usuario_id + "' name='" +  e.nombre_animador + " " + e.apellido_animador +cant_personal + "'><span class='text-muted'>(en uso)</span><label class='form-check-label' for='" + e.servicio_id + "personal" + cant_personal + "'>" + e.nombre_animador + "</label><br>";
                        //             // }
                        //          });
                        //          input += '</td>';
                        ...
                        //    var val = "<?php echo site_url('agendadeeventos/agendadeeventos_contr/valida_turno'); ?>";
                        //    val += "?estado_id=7";
                        //    val += "&cant_servicios=" + cant_servicios;
                        //    val += "&cant_personal=" + cant_personal;
                        //    val += "&cant_filas=" + cant_filas;
                        //    $("#modal_nuevo_evento_form").prop("action", val);
                        // }
                        //listarPersonal();
                           ...
                           success: function(servicios) {
                              ...
                              $.ajax({
                                 ...
                                 success: function(datos_personal) {
                                    var input = '';
                                    input += '<td>';
                                    datos_personal.personal.forEach(function(p) {
                                       var comprueba_personal = true;
                                       $cant_personal++;
                                       // datos_personal.personal.forEach(function(et) {
                                       //    // if (e.usuario_id === et.usuario_id) {
                                       //    //    comprueba_personal = false;
                                       //    //    return false;
                                       //    // }
                                       // });
                                       
                                       // } else {
                                       //       input += "<input class='form-check-input' type='checkbox' disabled value='" + e.usuario_id + "' name='" + e.tratamiento_id + "equipo" + cant_personal + "'><span class='text-muted'>(inactivo)</span><label class='form-check-label' for='" + e.servicio_id + "personal" + cant_personal + "'>" + e.nombre_animador + "</label><br>";
                                       // }
                                       input+="<input class='form-check-input' type='checkbox' value='"+p.usuario_id+"' name='"+p.servicio_id+"personal"+cant_personal+"'><label class='form-check-label' for='"+p.servicio_id+"personal"+cant_personal+"'>"+p.nombre_personal+"</label><br>";
                                       //input+= "<input class='form-check-input' type='checkbox' id='personal_checkbox'"+servicio_id+"' value='" + e.usuario_id + "' name='"+servicio_id+"personal'><label class='form-check-label' for='" + servicio_id + "personal" + cant_personal + "'>" + e.nombre_personal + "</label><br>";
                                    });
                                       input += "<td><input type='time' class='form-control border-primary'  name='"+servicio_id+"hora_inicio'></td>";
                                       input += "<td><input type='time' class='form-control border-primary hora_fin_servicio'  name='"+servicio_id+"hora_fin'></td>";
                                       input +="<td><input type='text' class='form-control border-primary' name='"+servicio_id+"observaciones'></td>";
                                       input +="<td><input type='number' class='form-control border-primary'  name='"+servicio_id+"valor'></td>";
                                    input += "<td><button title='Borrar servicio' type='button' class='btn btn-danger eliminarservicio'><i class='bi bi-trash'></i></botton></td>";
                                    input += '</td>';
                                    $("#"+id_fila).append(input);

                                    // Resto de tu lógica aquí
                                    //var personal_seleccionado = [];
                                    // $("#personal_checkbox" + servicio_id + ":checked").each(function() {
                                    //    personal_seleccionado.push($(this).val());
                                    // });

                                 }//success function datos personal
                              });//ajax listar personal
                        ...
                        val+="&cant_personal="+cant_personal;
                        val+="&cant_filas="+cant_filas;
                        val+="&personal_seleccionado="+personal_seleccionado;
                        $("#modal_nuevo_evento_form").prop("action",val);
                  ...
                  $("#modal_nuevo_evento").off("change", ".select_senias").on("change", ".select_senias", function(){
                     ...
                     if (fila.find('td:eq(0) select').val()==""){
                        ...
                        //compruebo los equipos que se están usando en esa fecha y hora
                        var datos_personal_eventos=null;
                        $.get("<?php echo site_url("agendadeeventos/Agendadeeventos_contr/equipos_fecha_hora"); ?>",
                           ...
                           function(datos_personal_eventos)
                           {
                              ...
                              $("#modal_nuevo_evento_form").prop("action",val);
                              //donde
                           },//function get comprueba_personal
                        "json")//get comprueba_personal
                        .fail(function(xhr, status, error) {
                           // La función .fail() se ejecutará si la solicitud AJAX falla
                        });//get comprueba_personal
                     }//else if val select != ""
                           ...
                           success : function(datos) {                                             
                                 ...
                                 datos_turno.tratamientos.forEach(function(t){
                                    ...
                                    $.ajax({
                                       ...
                                       success : function(datos_personal) {
                                          contenido_tabla+='<td>';
                                          datos_personal.equipos.forEach(function(e){
                                             cant_equipos++;
                                          ...
                                          });//foreach datos_personal.equipos
                                          contenido_tabla+='</td>';
                                       }//success equipos
                                    });//ajax listar_equipos
